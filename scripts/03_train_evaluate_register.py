...
def train_evaluate_register(preprocessing_run_id, C=1.0):
    ...
    with mlflow.start_run(run_name=f"logistic_regression_C_{C}"):
        ...
        # 5. ต้องผ่านทั้ง 2 เกณฑ์ถึงจะลงทะเบียน
        if acc >= ACCURACY_THRESHOLD and roc_auc >= ROC_AUC_THRESHOLD:
            print("ผ่านทั้ง 2 เกณฑ์ — กำลังลงทะเบียนโมเดล...")
            ...
            print(f"Set alias '@staging' -> version {registered_model.version}")
        else:
            print(f"ไม่ผ่านเกณฑ์ — accuracy={acc:.4f}, roc_auc={roc_auc:.4f}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/03_train_evaluate_register.py <run_id> [C]")
        sys.exit(1)
